...
public:
	...
	static void show( int n, int m, bool *a, bool *b ){
		for(int i=0; i<n; i++){
			for(int j=0; j<m; j++) printf("%d ",a[i*m+j]); printf("    = %d\n",b[i]);
		}
	}
	static bool Gauss( int n, bool *a, bool *b, bool *x ){
		int rank = 0;
		for(int j=0,r=0; j<n; j++){
			r = -1;
			for(int i=rank; i<n; i++) if( a[i*n+j] ){ r = i; break; }
			if( r==-1 ) continue;
			for(int k=j; k<n; k++) swap(a[rank*n+k],a[r*n+k]);
			swap( b[rank], b[r] );

			for(int i=rank+1; i<n; i++){
				if( !a[i*n+j] ) continue;
				for(int k=j; k<n; k++) a[i*n+k] ^= a[rank*n+k];
				b[i] ^= b[rank];
			}
			rank++;
		}
		for(int i=rank; i<n; i++) if( b[i] ) return false;
		memset( x, false, sizeof(bool)*n );
		for(int i=rank-1; i>=0; i--){
			int j = i;
			for( ; j<n; j++) if( a[i*n+j] ) break;
			x[j] = b[i];
			for(int k=0; k<i; k++) if( a[k*n+j] ) b[k] ^= b[i];
		}
		return true;
	}
	static void ans(){ // hackerrank   https://www.hackerrank.com/contests/nov13/challenges/FLIP
		int n, D;
		bool a[401*401], b[401], x[401];
			...
			for(int j=0,v; j<n; j++){
				...
				for(int y=0; y<n; y++){
					if( abs(i-x)+abs(j-y)<=D ){
						a[(i*n+j)*n*n+(x*n+y)] = true;
					}
				}
				}
			}
			}
			if( !Gauss(n*n,a,b,x) ) printf("Impossible\n");
			else{
				printf("Possible\n");
				int num = 0;
				for(int i=0; i<n*n; i++) if(x[i]) num++;
				printf("%d\n",num);
				for(int i=0; i<n; i++){
				for(int j=0; j<n; j++){
					if( x[i*n+j] ) printf("%d %d\n",i,j);
				}
				}
			}
		}
